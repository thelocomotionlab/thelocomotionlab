...
Tu construis l'outil qui permet à Valentin de traiter les dépôts d'archives, décrire les courses, générer, publier et envoyer les plans sans ligne de commande — et la page où chaque athlète lit son plan, l'amende et, s'il le veut, saisit son résultat. Deux documents font foi et sont à copier dans le dépôt avant tout : `docs/twin-tableau-de-bord-api.md` (le récapitulatif de l'API, fourni) et la maquette `Tableau de bord Locomotion Twin.dc.html` (fournie), à ranger sous `docs/design/twin-tableau-de-bord/`. Dans ces deux documents, le mot **« console » est remplacé partout par « tableau de bord »** ; c'est le seul changement à y faire.
