...
| `SMTP_USER` / `SMTP_PASS` | `services/atelier-api` (email récapitulatif d'inscription avec la fiche PDF) **et** `services/twin-engine` (« Envoyer » un plan, PDF joint ; la réponse à une demande d'athlète) | Identifiants SMTP du relais d'envoi (Brevo — la clé SMTP se génère dans Brevo → SMTP & API). À régénérer chez Brevo au moindre doute. `SMTP_HOST`/`SMTP_PORT`/`SMTP_FROM` les accompagnent (non secrets). | **`infra/.env`** (non versionné). |
| `TWIN_ADMIN_TOKEN` | `services/twin-engine` (toutes les routes `/twin/tableau-de-bord/*`) | Bearer token de l'outil de Valentin — il ouvre les dossiers de tous les athlètes. `openssl rand -hex 32`. **Vide → les routes n'existent pas (404)**, ce qui est le bon défaut pour un service déployé avant qu'on l'ait configuré. Il se colle une fois dans le tableau de bord et y reste (`localStorage`) — jamais dans une URL. C'est la **seule** serrure de l'API : Cloudflare Access ne couvre que les pages du site, sur un autre domaine, et ne peut pas couvrir l'API sans casser le préflight CORS. Le retirer ouvrirait toutes les données à l'internet. | **`infra/.env`** (non versionné). |
| `TWIN_KEYS_SECRET` | `services/twin-engine` (dérivation des deux clés de chaque plan, routes `/twin/plans/*`) | Secret HMAC-SHA256 d'où descendent la clé de partage et la clé privée de chaque plan. `openssl rand -hex 32`. Vide → aucune page d'athlète ne répond. **Le changer invalide TOUS les liens déjà envoyés** : les clés sont calculées à la demande, pas stockées — une rotation se paie d'un email à chaque athlète. Le CLI la lit aussi : le QR d'un rapport porte la clé privée de sa page, et un rapport fabriqué sans elle mène à une 404. | **`infra/.env`** (non versionné) ; la même valeur dans ton environnement de travail pour `pnpm course rapport`, **jamais** dans le dépôt. |
| `TWIN_INTERNAL_SECRET` | `services/twin-depot` → `services/twin-engine` (`POST /twin/internal/deposits`, réseau Docker) | Secret partagé qui authentifie l'appel du dépôt au moteur à la fin d'un upload. `openssl rand -hex 32`. Cette route n'est **jamais** routée par Caddy : le secret garde un appel qui ne sort pas de la machine, contre un conteneur compromis, pas contre l'internet. | **`infra/.env`** (non versionné). |
| `TWIN_DEPOT_ADMIN_TOKEN` | `services/twin-depot` (routes admin : listing, téléchargement et purge des dépôts) **et** `services/twin-engine`, qui s'en sert pour aller chercher une archive sur le dépôt | Bearer token qui donne accès aux archives d'entraînement — des données personnelles volumineuses (positions GPS, fréquence cardiaque). `openssl rand -hex 24`. Vide → routes admin désactivées. | **`infra/.env`** (non versionné). |
| `TWIN_API` / `TWIN_ADMIN_TOKEN` | `scripts/course.sh publier` (import dans le tableau de bord d'un plan fabriqué en local) | Le même jeton que les routes `/twin/tableau-de-bord/*` — ci-dessus. `TWIN_API` n'est pas un secret (défaut `https://api.thelocomotionlab.com`), elle n'est là que pour pointer un moteur local en développement. Jeton vide → `publier` dit ce qu'il aurait envoyé et ne touche à rien. | Ton environnement de travail (shell, `direnv`), **jamais** dans le dépôt. |
