...
| Carte de partage avec le **mauvais itinéraire** (autre massif, mauvais total, **0,0 %**, mauvais numéro de jour) | `live-config.json` PÉRIMÉ : site pas redéployé, ou cache devant Pages. `/live` ne le montre pas — la page embarque sa config au build, seul `live-journal` lit ce fichier | `curl -s https://thelocomotionlab.com/live-config.json` : si `referenceTrack`/`dateDebut` sont ceux d'hier → redéployer le site ; s'ils sont bons alors que le service se trompe → cache (le service ajoute désormais un cache-buster). `docker compose logs live-journal \| grep "\[og\] aventure"` dit sur quoi il travaille, et `grep HORS` signale le vécu hors cadre |
