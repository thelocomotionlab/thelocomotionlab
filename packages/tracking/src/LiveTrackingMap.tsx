// packages/tracking/src/LiveTrackingMap.tsx
//
// PRÉSENTATION du suivi LIVE (carte maplibre + profil altimétrique recharts).
...
import {
  ...
  formatDuration,
  getBoundsFromCoords,
  getCoordsFromGeoJSON,
} from "./utils";
import { useTrackingData } from "./useTrackingData";

// Tooltip du profil altimétrique. Au niveau module (référence stable) pour ne
// pas le recréer à chaque render (règle react-hooks/static-components).
type TooltipPayload = {
  active?: boolean;
...
export default function LiveTrackingMap({
  ...
  // Racine du composant : observée pour déclencher le fade in/out du bouton refresh.
  const rootRef = useRef<HTMLDivElement | null>(null);
  // Suit le style appliqué pour éviter le no-op au montage (warning maplibre).
  const appliedStyleRef = useRef<MapStyleId>(initialMapStyle || "osm");

  const [mapStyle, setMapStyle] = useState<MapStyleId>(initialMapStyle || "osm");
  const [showStyleMenu, setShowStyleMenu] = useState(false);
  const [showElevation, setShowElevation] = useState(true);
  const [isSmallScreen, setIsSmallScreen] = useState(false);
  const [showRefresh, setShowRefresh] = useState(false);
  // Devient true quand la carte a fini de charger : permet de pousser le tracé
  // dès que la carte est prête, même si la donnée est arrivée avant.
  ...
  // ---- COUCHE DONNÉES (aucun token, URL seule) ----
  const {
    coords,
    stats,
    elevationData,
  ...
  const ELEVATION_MIN = Number.isFinite(parsedElevationMin) ? parsedElevationMin : 400;
  const ELEVATION_MAX = Number.isFinite(parsedElevationMax) ? parsedElevationMax : 860;
  ...
  /* ---------- Bouton refresh : visibilité via IntersectionObserver ---------- */
  useEffect(() => {
  ...
  /* ---------- Restauration de la position de scroll après refresh ---------- */
    ...
    const initMap = () => {
      ...
      map.on("load", async () => {
        ...
        try {
          ...
          // La carte peut avoir été détruite (StrictMode, navigation) pendant l'await.
          if (!map.style) return;
          if (!res.ok) {
          ...
          if (!map._hasAutoFramed) {
            ...
            if (bounds) {
              try {
                map.fitBounds(bounds, { padding: 20, maxZoom: 16, duration: 2000 });
    ...
    map.once("styledata", () => {
      ...
      if (map._liveTrackGeoJSON && !map.getSource("live-track")) {
        map.addSource("live-track", {
          type: "geojson",
          data: asMapData(map._liveTrackGeoJSON),
        });
  ...
  /* ---------- 3) Application des données live sur la carte ---------- */
  // Remplace les mutations carte qui vivaient DANS le fetch : on réagit ici aux
  // nouvelles données (revision) et à la disponibilité de la carte (mapReady).
  useEffect(() => {
    const map = mapRef.current;
    ...
    const existing = map.getSource("live-track") as GeoJSONSource | undefined;
    if (existing) {
      existing.setData(asMapData(geojson));
    } else {
      map.addSource("live-track", { type: "geojson", data: asMapData(geojson) });
      map.addLayer({
        id: "live-track-line",
        type: "line",
        source: "live-track",
    ...
    if (!map._hasAutoFramed) {
      ...
      if (bounds) {
        try {
          map.fitBounds(bounds, { padding: 20, maxZoom: 18, duration: 2000 });
          map._hasAutoFramed = true;
        } catch (e) {
  ...
  return (
    <div ref={rootRef} className="flex flex-col items-center w-full py-6 px-3 sm:px-6 gap-3">
      ...
      <button
        ...
        className={`
          fixed z-50 bottom-5 left-5 sm:bottom-[4rem] sm:left-auto sm:right-6
