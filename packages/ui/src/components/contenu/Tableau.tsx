...
// une cellule vide par un tiret cadratin plutôt que par du blanc.
        ...
        <tr>
          {colonnes.map((colonne, index) => (
            <th
              key={colonne}
              scope="col"
              className={`border-b border-brand-hairline pb-2 font-mono text-xxs font-semibold uppercase tracking-etiquette text-brand-muted ${
        ...
        {lignes.map((ligne, rang) => (
          <tr key={rang}>
            {ligne.map((cellule, index) => (
              <td
                key={index}
                className={`border-b border-brand-grid py-2.5 align-top ${
                  estNumerique(index) ? "text-right font-mono tabular-nums" : "font-lora"
