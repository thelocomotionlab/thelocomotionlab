 ...
 * - Projets  : mtime fichier (updatedAt) = clé de tri principale
 */
...
/**
 * Projets : tri par dernière modification (comportement actuel conservé).
 */
function sortProjectsByUpdatedAt(a, b) {
  const au = a.updatedAt?.getTime?.() ?? 0;
  const bu = b.updatedAt?.getTime?.() ?? 0;
  if (bu !== au) return bu - au;

  // Tie-breaker : éventuellement la date frontmatter si besoin
  const ad = a.date?.getTime?.() ?? 0;
  const bd = b.date?.getTime?.() ?? 0;
  return bd - ad;
}
